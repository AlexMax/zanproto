...
function RCONSocket() {
	...
	this._dgram.on('message', this._router.bind(this));
	this._dgram.bind();
}
util.inherits(RCONSocket, events.EventEmitter);

// Underlying datagram socket
RCONSocket.prototype._dgram = null;

// True if RCONSocket is connected, otherwise false
RCONSocket.prototype._connected = null;

// True if RCONSocket is connecting, otherwise false
RCONSocket.prototype._connecting = null;

// Set to the password associated with the RCON connection
RCONSocket.prototype._password = null;

// Set to the port associated with the RCON connection
RCONSocket.prototype._port = null;

// Set to the address associated with the RCON connection
RCONSocket.prototype._address = null;

// A timer that pongs the server every five seconds
RCONSocket.prototype._pongTimer = null;

// Router, which handles all incoming messages
RCONSocket.prototype._router = function(msg, rinfo) {
...
RCONSocket.prototype.connect = function(password, port, host, callback) {
	var self = this;
...
RCONSocket.prototype.disconnect = function() {
	// We're not connected.
	if (this._connected === false) {
		return;
	}
